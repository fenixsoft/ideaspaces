...
现代数据往往具有很高的维度。一张 100×100 的图像就是 10000 维向量；一篇文档可能有数千个词汇；基因表达数据可能有数万个特征。
...
给定 $n$ 个样本 $\{x_1, \ldots, x_n\}$，$x_i \in \mathbb{R}^d$。PCA 的目标是找到一个投影方向 $w$（$||w||=1$），使得投影后的方差最大化：
